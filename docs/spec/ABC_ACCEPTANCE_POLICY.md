...
- replacement of redundant `%@mks` hints by standard ABC surface syntax when roundtrip-relevant semantics remain intact
...
- repeat / transpose / measure semantics needed by `abc-player` roundtrip are lost unexpectedly
...
- repeat / measure / transpose roundtrip behavior, whether preserved through standard ABC syntax or retained `%@mks` hints
...
- `roundtrip_structure`
  ...
  - repeat / measure / transpose semantics
- `player_output`
...
If a sync breaks only downstream assertions that were overspecified around `%@mks` or exact ABC spelling, the default interpretation is:

- update `abc-player` acceptance first
- then re-evaluate whether any real player-visible regression remains
